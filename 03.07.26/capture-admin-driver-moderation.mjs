...
//      ADMIN_USER=... ADMIN_PASS=... node daily-report/03.07.26/capture-admin-driver-moderation.mjs
// Read-only: opens the Drivers moderation tab and screenshots it. Does NOT submit approve/reject.
...
async function run(vp, suffix) {
  ...
  try {
    ...
    await page.waitForTimeout(2500);
    const tab = page.getByRole('tab', { name: /driver/i });
    if (await tab.count()) { await tab.first().click(); await page.waitForTimeout(2500); }
    else { console.log('WARN drivers tab not found ' + suffix); }
    await page.screenshot({ path: join(IMG_DIR, 'admin-driver-moderation-' + suffix + '.png'), fullPage: false });
    console.log('OK admin-driver-moderation-' + suffix);
  } catch (e) {
    console.log('FAIL ' + suffix + ': ' + e.message.slice(0, 160));
    await page.screenshot({ path: join(IMG_DIR, 'admin-driver-moderation-' + suffix + '.png') }).catch(() => {});
  } finally {
    await ctx.close();
  }
}
